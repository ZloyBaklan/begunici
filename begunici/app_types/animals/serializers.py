...
class MakerSerializer(AnimalBaseSerializer):
    ...
    def get_place_history(self, obj):
        place_movements = PlaceMovement.objects.filter(tag=obj.tag).order_by(
    ...
    def get_status_history(self, obj):
        status_history = StatusHistory.objects.filter(tag=obj.tag).order_by(
            "-new_status__date_of_status"
        )
        return [
            {
...
class MakerChildSerializer(serializers.ModelSerializer):
    ...
    archive_status = serializers.CharField(
        source="animal_status.status_type", read_only=True
    )  # Статус
    archive_date = serializers.DateField(
        source="animal_status.date_of_status", read_only=True
    )  # Дата статуса
...
class RamSerializer(AnimalBaseSerializer):
    ...
    def get_children(self, obj):
        children = obj.get_children()
        return RamChildSerializer(children, many=True).data
...
class EweSerializer(AnimalBaseSerializer):
    ...
    def get_children(self, obj):
        children = obj.get_children()
        return EweChildSerializer(children, many=True).data
...
class RamChildSerializer(serializers.ModelSerializer):
    link = serializers.SerializerMethodField()
    tag_number = serializers.CharField(source="tag.tag_number", read_only=True)
    animal_type = serializers.CharField(source="tag.animal_type", read_only=True)
    is_archived = serializers.BooleanField(read_only=True)
...
class EweChildSerializer(serializers.ModelSerializer):
    link = serializers.SerializerMethodField()
    tag_number = serializers.CharField(source="tag.tag_number", read_only=True)
    animal_type = serializers.CharField(source="tag.animal_type", read_only=True)
    is_archived = serializers.BooleanField(read_only=True)
...
class ArchiveAnimalSerializer(serializers.Serializer):
    ...
    date_of_status = serializers.DateField(
