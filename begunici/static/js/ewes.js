...
async function pruneStaleSelectedEwes() {
    const staleTags = [];

    await Promise.all(Array.from(selectedEwes).map(async (tagNumber) => {
        try {
            const response = await fetch(`/animals/ewe/${encodeURIComponent(tagNumber)}/`);
            if (!response.ok) {
            ...
            console.warn('Не удалось проверить выбранную ярку перед архивацией:', tagNumber, error);
        }
    }));

    if (staleTags.length > 0) {
        staleTags.forEach(tagNumber => selectedEwes.delete(tagNumber));
        saveSelectedEwes();
        document.querySelectorAll('input[name="selectedEwes"]').forEach(checkbox => {
...
async function loadStatuses() {
    try {
        const response = await apiRequest('/veterinary/api/status/?exclude_archive=1&page_size=100&animal_type=Ewe');
...
async function openArchiveModal() {
    const staleTags = await pruneStaleSelectedEwes();
    ...
    if (archiveAnimals.length === 0) {
        alert(staleTags.length > 0
            ? 'Выбранные ярки уже перенесены в архив или не найдены. Выбор очищен.'
