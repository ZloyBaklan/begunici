...
async function loadAnimalStatuses() {
    try {
        const response = await apiRequest('/veterinary/api/status/?exclude_archive=1&page_size=100&animal_type=Maker');
...
async function saveMaker() {
    ...
    try {
        ...
        alert('Ошибка при создании барана-производителя: ' + (error.message || 'Проверьте корректность введенных данных'));
    }
}
...
async function applyArchiveStatus() {
    const selected = Array.from(selectedMakers)
        .filter(tag => tag && String(tag).trim())
        .map(tag => ({ animalType: 'maker', tagNumber: String(tag).trim() }));
